...
export function validatePrompt(prompt, exercise) {
  ...
  const pass = normalizedScore >= 40; // Lowered from 60 — encourage exploration

  const feedback = pass
    ? (exercise.successMessage || getSuccessFeedback(normalizedScore))
    : (issues.length > 0 ? issues[0] : (exercise.failMessage || 'Good start! Try adding a bit more detail.'));
...
export function gradePrompt(prompt) {
  ...
  // Encouragement tiers instead of harsh letter grades
  const grade = overall >= 90 ? '🌟' : overall >= 75 ? '⚡' : overall >= 55 ? '👍' : overall >= 35 ? '🌱' : '→';
...
function getSuccessFeedback(score) {
  ...
  if (score >= 60) return 'Nice work! That covers the essentials. Want to try pushing it further?';
  if (score >= 40) return 'Good start! You\'re getting the hang of it. A few more details would make it even stronger.';
  return 'You\'re on the right track — keep building on this.';
}

function getGradeFeedback(dimensions, grade) {
  if (grade === '🌟') return 'Outstanding prompt! This would get excellent AI output.';
...
// ── Attempt Tracking (Wibisono momentum) ──
// Carries forward trajectory so feedback shows improvement, not just static score.

const ATTEMPTS_KEY = 'likeone-exercise-attempts';
...
function saveAttemptHistory(history) {
  if (typeof window === 'undefined') return;
  try { localStorage.setItem(ATTEMPTS_KEY, JSON.stringify(history)); } catch {}
}

/**
 * Track an exercise attempt and return momentum-aware feedback.
 * Returns { attempt: number, delta: number, momentum: string, improving: string[] }
 */
export function trackAttempt(exerciseId, score, dimensions) {
  const history = getAttemptHistory();
  if (!history[exerciseId]) history[exerciseId] = [];

  const prev = history[exerciseId][history[exerciseId].length - 1];
  history[exerciseId].push({ score, dimensions, ts: Date.now() });

  // Keep last 5 attempts per exercise
  if (history[exerciseId].length > 5) history[exerciseId] = history[exerciseId].slice(-5);
  saveAttemptHistory(history);

  const attempt = history[exerciseId].length;
  if (!prev) return { attempt, delta: 0, momentum: '', improving: [] };
  ...
  if (prev.dimensions) {
    for (const key of Object.keys(dimensions || {})) {
      const d = (dimensions[key] || 0) - (prev.dimensions[key] || 0);
      if (d > 10) improving.push(key);
      else if (d < -10) declining.push(key);
    }
  }

  let momentum = '';
  if (delta > 15) momentum = 'Big improvement! Your prompt is getting much stronger.';
  else if (delta > 0) momentum = 'Getting better — keep refining.';
  else if (delta === 0) momentum = 'Same score. Try a different angle.';
  else momentum = 'Score dipped — check what you changed.';

  if (improving.length > 0) {
    const names = improving.map(k => QUALITY_DIMENSIONS.find(d => d.key === k)?.label || k);
    momentum += ` ${names.join(' and ')} improved!`;
  }

  return { attempt, delta, momentum, improving };
}

export function getAttemptCount(exerciseId) {
  const history = getAttemptHistory();
  return (history[exerciseId] || []).length;
}
