...
[RAG & Vector Search](/academy/rag-vector-search/)
  ...
  Test your knowledge across all 9 lessons. This assessment covers embeddings, chunking, the RAG pipeline, prompt augmentation, hybrid search, evaluation, and advanced patterns. A score of 80% or higher means you are ready to build production RAG systems.
    ...
    Before you take the final assessment, revisit the core ideas from each module. These are the concepts that separate someone who has heard of RAG from someone who can build and maintain a production system.
      ...
      **Prompt Augmentation and Hallucination Prevention**
    ...
    These are the errors that trip up even experienced engineers. Each one is easy to make and hard to diagnose because the system still produces answers — just bad ones. Knowing these patterns lets you avoid them in your own builds and diagnose them quickly when reviewing others.
        ...
        **3. No Grounding Instructions in the Prompt**
        ...
        **5. Pure Vector Search for Exact-Match Domains**
    ...
    Before deploying any RAG system to production, walk through this checklist. Each item addresses a failure mode that has caused real production incidents. A system that passes all of these checks is ready for users. One that fails any of them has a ticking time bomb.
            ...
            **Hybrid Search Enabled for Exact-Term Domains**
...
**Q1: What does an embedding model produce?**
    ...
    B. A list of keywords extracted from the text
  ✓ C. A fixed-length vector of floating-point numbers that captures semantic meaning
    D. A JSON object with sentiment analysis scores
  *Embedding models convert text into dense vectors (lists of numbers) where similar meanings map to nearby points in the vector space. These vectors are the foundation of semantic search.*
...
**Q2: Why is cosine similarity preferred over Euclidean distance for comparing embeddings?**
  ...
  ✓ B. Cosine similarity measures the angle between vectors, making it magnitude-independent
    C. Euclidean distance only works in 2D
    D. Cosine similarity returns values between -1 and 1 which is easier to read
  *Cosine similarity measures the angle between vectors, ignoring their magnitude. This means a long vector and a short vector pointing in the same direction are considered similar — which is what we want when comparing meaning, not vector length.*

**Q3: What is the main tradeoff when choosing a smaller chunk size for RAG?**
    A. Smaller chunks are more expensive to embed
  ✓ B. Smaller chunks provide more precise retrieval but may lack surrounding context
    C. Smaller chunks cause the vector database to run slower
    D. Smaller chunks always produce worse results
  *Small chunks give precise retrieval (the matched text is highly relevant) but may miss the context needed to understand that text. Overlap helps mitigate this.*

**Q4: In the RAG pipeline, what happens AFTER the relevant chunks are retrieved from the vector database?**
    A. The chunks are fed back into the embedding model
  ✓ B. The chunks are inserted into the LLM prompt as context alongside the original question
    C. The chunks are stored in a new database
    D. The chunks are sent directly to the user as the answer
  *This is the Augmentation step in RAG. Retrieved chunks become the context in a carefully crafted prompt that tells the LLM to answer based only on the provided context.*

**Q5: What is the key advantage of hybrid search over pure vector search?**
    A. Hybrid search is always faster
  ✓ B. Hybrid search combines keyword precision with semantic understanding
    C. Hybrid search uses less storage
    D. Hybrid search does not require an embedding model
  *Hybrid search combines BM25 (keyword) and vector (semantic) search. Keywords catch exact matches like error codes that vector search might miss, while vectors catch meaning-based matches. Together they cover more ground.*

**Q6: Which metric measures whether a RAG answer contains ONLY information from the retrieved context?**
    A. Relevance
    B. Completeness
  ✓ C. Faithfulness (Groundedness)
    D. Precision
  *Faithfulness (also called Groundedness) measures whether every claim in the answer is supported by the retrieved context. A faithfulness score of 1.0 means zero hallucination.*

**Q7: What distinguishes Self-RAG from standard RAG?**
    A. Self-RAG uses a different embedding model
  ✓ B. Self-RAG lets the LLM decide whether retrieval is needed and self-evaluates answer quality
    C. Self-RAG retrieves from multiple databases simultaneously
    D. Self-RAG caches all previous queries for faster responses
  *Self-RAG adds decision-making: the LLM decides whether retrieval is needed (saving cost on simple questions), then self-evaluates answer quality after generating.*

**Q8: You are building a RAG system for a legal firm with exact statute numbers. What search strategy?**
    A. Pure vector search — it understands meaning better
    B. Pure keyword search — exact matching is all you need
  ✓ C. Hybrid search with higher keyword weight — statutes need exact matching, context needs semantic understanding
    D. No search — just use a large context window LLM
  *Legal documents require exact statute references (keyword strength) combined with understanding legal concepts in natural language (semantic strength). Hybrid with higher keyword weight is ideal.*

**Q9: Your RAG evaluation shows: Relevance 5/5, Faithfulness 2/5, Completeness 4/5. What is the problem and how do you fix it?**
    A. Retrieval is broken — change the embedding model
  ✓ B. The LLM is hallucinating — strengthen grounding instructions, lower temperature, add citation requirements
    C. Chunks are too small — increase chunk size
    D. The knowledge base needs more documents
  *High relevance means the right context is being retrieved. Low faithfulness means the LLM is adding claims not in the context — hallucinating. The fix is in the generation step: stronger grounding instructions, lower temperature, and explicit citation requirements.*

**Q10: When should you use Agentic RAG over basic RAG?**
    A. Always — it is strictly better
  ✓ B. When the question requires planning across multiple knowledge bases and iterative retrieval
    C. When you want to save on API costs
    D. When the question is simple and direct
  *Agentic RAG is the most powerful but also the most expensive and complex pattern. Use it only when queries require multi-source planning and iterative retrieval. For simple queries, basic RAG is faster, cheaper, and easier to maintain.*



### RAG Course — Complete Review

**Card 1:**
Front: RAG
Back: Retrieval-Augmented Generation. Fetch relevant documents at query time and provide them as context to an LLM, grounding answers in specific retrieved knowledge instead of training memory.

**Card 2:**
Front: Embedding
Back: A fixed-length vector of numbers representing the semantic meaning of text. Similar meanings produce close vectors. Created by neural networks trained with contrastive learning.

**Card 3:**
Front: Vector Database
Back: A database optimized for storing and searching high-dimensional vectors by similarity. Uses HNSW indexes for millisecond searches across millions of vectors.

**Card 4:**
Front: Chunking
Back: Splitting documents into smaller segments before embedding. Key parameters: chunk size (100-500 words) and overlap (10-20%). Strategy depends on document type.

**Card 5:**
Front: Cosine Similarity
Back: Measures the angle between two vectors. Range: -1 to 1. Magnitude-independent — focuses on direction (meaning), not length.

**Card 6:**
Front: HNSW Index
Back: Hierarchical Navigable Small World — a graph-based index enabling approximate nearest neighbor search in sub-linear time. The reason vector search is fast.

**Card 7:**
Front: Augmented Prompt
Back: A prompt containing retrieved chunks as context plus grounding instructions. The bridge between retrieval and generation in RAG.

**Card 8:**
Front: Hybrid Search
Back: Combining BM25 keyword search with vector semantic search, weighted by an alpha parameter. Handles both exact terms and meaning-based queries.

**Card 9:**
Front: Faithfulness
Back: A RAG evaluation metric: every claim in the answer is supported by retrieved context. Score 1.0 = zero hallucination. The most critical safety metric.

**Card 10:**
Front: Agentic RAG
Back: An autonomous agent that plans its own multi-source retrieval strategy for complex queries. The most powerful but most expensive RAG pattern.
