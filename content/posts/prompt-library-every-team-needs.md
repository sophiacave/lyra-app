...
1. **Findable in 10 seconds.** Organized by task, not by tool or category.
2. **Copy-paste ready.** Includes placeholders in brackets so anyone can fill in their specifics.
3. **Tested and maintained.** Someone owns it. Bad prompts get removed. Good prompts get added.
...
1. **Start small.** Launch with 10 prompts, not 50. Add based on demand.
2. **Celebrate saves.** When someone shares a great prompt in Slack, add it to the library publicly. Make contribution visible.
3. **Review monthly.** Remove any prompt nobody used in 30 days. A smaller, sharper library beats an exhaustive one.
