...
<span class="kw">tsc</span> my-server.ts && <span class="kw">node</span> my-server.js</div>
  ...
  <div data-learn="MatchConnect" data-props='{"title":"Match Connection Phase to What Happens","instruction":"Tap one on the left, then its match on the right","pairs":[{"left":"Phase 1: Install","right":"Get the MCP server code onto your machine via npm or local script"},{"left":"Phase 2: Configure","right":"Edit claude_desktop_config.json with command, args, and env"},{"left":"Phase 3: Discover","right":"Claude sends initialize and tools/list to learn available capabilities"},{"left":"Phase 4: Use","right":"Claude autonomously invokes tools based on user requests"},{"left":"env field","right":"Secret values like API keys that stay on your machine and never leave"}]}'></div>
