...
server.tool(<span class="str">"read_file"</span>, { path: z.string() }, <span class="kw">async</span> ({ path }) => {
  ...
  <div data-learn="SortStack" data-props='{"title":"Security Priority Order","instruction":"Arrange from most foundational to most specialized","items":["Principle of Least Privilege (limit what damage is possible)","Input Validation (reject dangerous inputs)","Error Sanitization (never leak internals)","Rate Limiting (prevent runaway loops)","Audit Logging (know what happened)","Human-in-the-Loop (approve destructive actions)"]}'></div>
