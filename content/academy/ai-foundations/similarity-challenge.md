...
<div data-learn="SortStack" data-props='{"title":"Order the AI Pipeline","instruction":"Arrange these steps in the order they happen when you send a message to Claude","items":["Your text is split into tokens (subword chunks)","Tokens are converted to embedding vectors (numbers)","Vectors flow through billions of neural network parameters (forward pass)","The model predicts the probability of each possible next token","Temperature sampling selects the actual next token","The token is converted back to text and displayed to you"]}'></div>
