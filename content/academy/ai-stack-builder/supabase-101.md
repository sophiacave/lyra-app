...
<div data-learn="MatchConnect" data-props='{"title":"Match Supabase Concepts","instruction":"Tap one on the left, then its match on the right","pairs":[{"left":"Anon key","right":"Public key safe for frontend — access controlled by RLS policies"},{"left":"Service role key","right":"Secret key that bypasses all RLS — only for server-side code"},{"left":"RLS (Row Level Security)","right":"Postgres feature that controls who can read or write each row"},{"left":"JSONB","right":"Binary JSON type that supports indexing and querying inside the structure"},{"left":"pgvector","right":"Extension for storing AI embeddings and doing semantic similarity search"}]}'></div>
