...
<div class="lesson-section">
  ...
  <div data-learn="MatchConnect" data-props='{"title":"Match Each Build Step to What It Accomplishes","instruction":"Tap one on the left, then its match on the right","pairs":[{"left":"Map the workflow","right":"Understand the end goal and every transformation between input and output before designing agents"},{"left":"Identify agent boundaries","right":"Find where skills change and group related steps — start with 2-3 agents, not 8"},{"left":"Write agent specifications","right":"Define role, system prompt, inputs, outputs, model tier, and failure behavior for each agent"},{"left":"Test in isolation then integrate","right":"Run 20+ test cases per agent alone, then wire together and watch the handoff seams"}]}'></div>
