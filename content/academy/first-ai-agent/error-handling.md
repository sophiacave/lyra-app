...
<div data-learn="SortStack" data-props='{"title":"Tool Failure Recovery Order","instruction":"Arrange these steps in the correct order for handling a tool failure","items":["Detect the error and classify it (transient vs permanent)","Retry with exponential backoff (1s, 2s, 4s)","Try an alternative fallback tool","Return the error to Claude and let it adapt","Escalate to human with full context if all options exhausted"]}'></div>
