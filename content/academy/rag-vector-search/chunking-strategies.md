...
<span style="color:#c084fc">def</span> <span style="color:#38bdf8">semantic_chunk</span>(text, threshold=<span style="color:#fbbf24">0.75</span>):
  ...
  <div data-learn="SortStack" data-props='{"title":"Document Processing Pipeline — Order the Steps","instruction":"Arrange these document processing steps in the correct order","items":["Load raw documents (PDFs, web pages, text files)","Clean and normalize text (remove headers, footers, formatting artifacts)","Split documents into chunks using chosen strategy and overlap","Attach metadata to each chunk (source, section, page number)","Embed each chunk using an embedding model","Store chunk vectors + metadata in the vector database"]}'></div>
