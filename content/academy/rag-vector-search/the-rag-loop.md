...
$$;</code></pre>
  ...
  <div data-learn="SortStack" data-props='{"title":"The RAG Pipeline — Order the Steps","instruction":"Arrange the six RAG steps in the correct sequence","items":["Receive user query in natural language","Embed the query into a vector using the same model as the documents","Search the vector database for similar chunks","Retrieve the top-K matching chunks with similarity scores","Augment the prompt with retrieved context and grounding instructions","Send to LLM and generate grounded answer"]}'></div>
